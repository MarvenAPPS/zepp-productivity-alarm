...
Page({
  ...
  loadHistory() {
    ...
    this.state.history = storage.get('history', []);
  ...
  renderUI() {
    ...
    createWidget(widget.TEXT, {
      ...
      w: 466,
      h: 40,
      text: `Balance: ${this.state.balance} pts`,
      text_size: 22,
      color: 0xFFFFFF,
      align_h: align.CENTER_H,
      align_v: align.CENTER_V
  ...
  redeemPoints() {
    if (this.state.balance < 1000) {
      showToast({ text: 'Minimum 1000 points required' });
      return;
    }

    // Calculate redeemable amount (multiple of 1000)
    ...
    storage.set('history', this.state.history);
    storage.set('balance', newBalance);
    this.state.balance = newBalance;
    
    showToast({ text: `Redeemed ${redeemable} points!` });
    
    // Sync with server
    const sync = require('../utils/sync');
    sync.syncBalance(newBalance, historyEntry);
